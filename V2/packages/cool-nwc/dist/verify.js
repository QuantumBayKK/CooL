...
import { bindingHash, coreOf, recordLeafData, recordSigningMessage, sthCore, sthSigningMessage, } from "./record.js";
...
function formatAjvErrors(errors) {
    if (!errors || errors.length === 0)
        return ["receipt does not conform to cool.receipt.v1"];
...
function formatFailureVerdict(reasons) {
    const fail = {
        status: "fail",
        detail: "receipt failed cool.receipt.v1 schema validation",
    };
    const absent = { status: "absent", detail: "not evaluated — receipt is malformed" };
    return {
        ok: false,
...
export async function verifyReceipt(receipt, options = {}) {
    ...
    const attestation = { status: "mock", detail: ATTESTATION_DETAIL };
    // Step 7 — anchor: ABSENT in this build, NEVER a pass.
    const anchor = { status: "absent", detail: ANCHOR_DETAIL };
    const checks = {
        binding,
        signature,
        inclusion,
...
function verifyInclusionDomain(r, reasons) {
    ...
    const rootOk = verifyInclusion(leafHashValue, inc.leaf_index, sth.tree_size, auditPath, expectedRoot);
    if (!rootOk) {
...
function verifyWitnessesDomain(r, threshold) {
    ...
    const selfNote = selfCount > 0 ? ` (${selfCount} self-signature${selfCount === 1 ? "" : "s"}, not counted)` : "";
